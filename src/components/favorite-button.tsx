...
import { addFavorite, removeFavorite, isFavorited } from '@/app/utils/favorites'
import { favoriteItem } from '@/app/lib/actions'

export default function FavoriteButton({ itemId, favs }: { itemId: number, favs: number }) {
  const [favorited, setFavorited] = useState<boolean>(false)
  const [favsNo, setFavsNo] = useState(favs);
  const [loading, setLoading] = useState(false)

  useEffect(() => {
    // run only on client
    setFavorited(isFavorited(itemId))
  }, [itemId])
  ...
  const handleFavorite = async () => {
    ...
    // optimistic update
    setFavorited(true)
    setFavsNo(prevFavsNo => prevFavsNo + 1)
    addFavorite(itemId)
    setLoading(true)
    ...
    try {
      ...
      console.error('Failed to favorite on server:', err)
  ...
  return (
    ...
    <span onClick={handleFavorite}>
      {favorited ? (
        <FavoriteFilled className='w-6 h-6 text-rose-500'/>
      ):(
        <Favorite className='w-6 h-6 text-gray-500'/>
      )}
    </span>
    <span className="ml-1 text-gray-500 text-sm italic">{favsNo} <span className="italic text-sm">favs</span></span>
    </>
  )
}
